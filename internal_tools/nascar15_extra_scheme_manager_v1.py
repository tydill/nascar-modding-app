...
def catalog(game_arg: str | Path, state_path: str | Path) -> dict[str, Any]:
    ...
    used_all.update(
        int(x.get("uid")) for x in state.get("orphaned_schemes", [])
        if x.get("uid") is not None
    )
    used_all.update(int(x) for x in state.get("retired_uids", []) if x is not None)
    for r in base.records_of(ctx, "LIVERIE_c"):
        ...
        item = {
            ...
            "preview_method": managed.get(int(uid), {}).get("preview_method"),
            "thumbnail_game_safe": bool(
                managed.get(int(uid), {}).get("thumbnail_game_safe")
                or managed.get(int(uid), {}).get("preview_status") == "safe_clone"
                or managed.get(int(uid), {}).get("preview_method") in ("native_expand_v25_raw", "raw_clone_repair_v26",
                    "same_bank_native_alias_custom_v27",
                    "same_bank_native_alias_custom_v27_new",
                    "same_bank_identity_repair_v27")
            ) if int(uid) in managed else True,
...
def recipient_world_uid(ctx: Any, driver_uid: int) -> int | None:
    ...
    stock DLC livery belonging to a different driver. Donor 25580 is an
    Indianapolis record, so every app-created scheme was stamped
    S_WORLD_INDIANAPOLIS_SS. Two consequences, both observed in-game:
      ...
      * loading THAT track crashed, because the record was enumerated for an
        event it had no valid entry for - a clean install loaded Indianapolis
        fine until the first scheme was created
...
def inspect_managed_database(game_arg: str | Path, state_path: str | Path) -> dict[str, Any]:
    """Audit the one-to-one wiring between saved scheme state, live DB records,
    and the indexed SD/HD asset names.

    This intentionally checks identities that older Paint System checks missed:
    UID ↔ ScriptName ↔ Driver ↔ LIVERY_<ScriptName>.ARC must all describe the
    same scheme.  A swapped state row can leave every individual file parseable
    while the front end resolves a livery to another scheme's assets.
    """
    game = base.detect_game(str(game_arg))
    state = load_state(state_path)
    ...
    live_rows: dict[int, list[Any]] = {}
    for rec in base.records_of(live, "LIVERIE_c"):
        live_rows.setdefault(int(base.pointer_int(live, rec.uid)), []).append(rec)

    arc2, cdf2_path = v06.archive2_paths(game)
    cdf2 = v06.parse_cdf_v6(cdf2_path.read_bytes())
    asset_names = {str(cdf2.basename(x)) for x in cdf2.files}
    donor = base.find_record(live, "LIVERIE_c", int(PROVEN_EXTRA_DONOR_UID))
    donor_sig = _record_sig(live, donor)

    issues: list[dict[str, Any]] = []
    rows: list[dict[str, Any]] = []
    seen_uid: dict[int, int] = {}
    seen_script: dict[str, int] = {}
    seen_asset: dict[str, int] = {}

    def issue(code: str, uid: int, detail: str, *, repairable: bool = True,
              fatal_possible: bool = True) -> None:
        issues.append({"code": code, "uid": int(uid), "detail": detail,
                       "repairable": bool(repairable),
                       "fatal_possible": bool(fatal_possible)})

    for item in active:
        uid = int(item["uid"])
        state_driver = int(item.get("driver_uid", -1))
        state_script = str(item.get("script_name") or "")
        state_sd = str(item.get("sd_entry") or "")
        state_hd = str(item.get("hd_entry") or "")
        recs = live_rows.get(uid, [])
        live_script = None
        live_driver = None
        invalid_fields: list[str] = []
        if len(recs) != 1:
            issue("live_livery_missing" if not recs else "duplicate_live_livery_uid", uid,
                  f"UID {uid} has {len(recs)} live LIVERIE_c record(s).",
                  repairable=not recs)
        else:
            rec = recs[0]
            live_script = str(base.display(live, rec.fields.get("ScriptName")) or "")
            live_driver = int(base.field_uid(live, rec, "Driver"))
            live_sig = _record_sig(live, rec)
            for field in live.schemas["LIVERIE_c"].fields:
                if field in ("UID", "Driver", "ScriptName"):
                    continue
                if donor_sig.get(field) != live_sig.get(field):
                    invalid_fields.append(field)
            if invalid_fields:
                issue("live_livery_donor_shape_mismatch", uid,
                      "Created record uses a different native donor shape in field(s): " +
                      ", ".join(invalid_fields) + ". This is legacy construction debt, not a proven fatal when UID/Driver/ScriptName/assets agree.",
                      repairable=True, fatal_possible=False)

        canonical_script = live_script or state_script
        canonical_driver = live_driver if live_driver is not None else state_driver
        canonical_sd = f"LIVERY_{canonical_script}.ARC" if canonical_script else ""
        canonical_hd = f"HDLIVERY_{canonical_script}.ARC" if canonical_script else ""

        if uid in seen_uid:
            issue("duplicate_state_uid", uid,
                  f"Saved state contains UID {uid} more than once.", repairable=False)
        seen_uid[uid] = seen_uid.get(uid, 0) + 1
        if state_script:
            key = state_script.casefold()
            if key in seen_script and seen_script[key] != uid:
                issue("duplicate_state_script", uid,
                      f"ScriptName {state_script} is also assigned to UID {seen_script[key]}.",
                      repairable=False)
            seen_script[key] = uid
        for name in (state_sd, state_hd):
            if not name:
                continue
            key = name.casefold()
            if key in seen_asset and seen_asset[key] != uid:
                issue("duplicate_state_asset", uid,
                      f"Asset {name} is also assigned to UID {seen_asset[key]}.",
                      repairable=False)
            seen_asset[key] = uid

        m = re.match(r"^CUSTOM_(\d+)_(\d+)_", state_script, re.I)
        if m and int(m.group(2)) != uid:
            issue("state_script_uid_mismatch", uid,
                  f"Saved ScriptName {state_script} embeds UID {m.group(2)}, not {uid}. This is legacy naming debt; live UID/asset links remain authoritative.",
                  fatal_possible=False)
        if m and int(m.group(1)) != state_driver:
            issue("state_script_driver_mismatch", uid,
                  f"Saved ScriptName {state_script} embeds driver {m.group(1)}, not {state_driver}. This is legacy naming debt unless the live Driver field also disagrees.",
                  fatal_possible=False)
        if live_script is not None and state_script != live_script:
            issue("state_live_script_mismatch", uid,
                  f"Saved ScriptName is {state_script or 'missing'}; live UID {uid} uses {live_script}.")
        if live_driver is not None and state_driver != live_driver:
            issue("state_live_driver_mismatch", uid,
                  f"Saved driver is {state_driver}; live UID {uid} belongs to {live_driver}.")
        if state_sd != canonical_sd or state_hd != canonical_hd:
            issue("state_asset_name_mismatch", uid,
                  f"Saved assets are {state_sd or 'missing'} / {state_hd or 'missing'}; "
                  f"live ScriptName requires {canonical_sd} / {canonical_hd}.")
        missing_assets = [x for x in (canonical_sd, canonical_hd) if x and x not in asset_names]
        if missing_assets:
            issue("canonical_assets_missing", uid,
                  "Indexed paint asset(s) missing: " + ", ".join(missing_assets))

        rows.append({
            "uid": uid,
            "state_driver_uid": state_driver,
            "state_script_name": state_script,
            "state_sd_entry": state_sd,
            "state_hd_entry": state_hd,
            "live_record_count": len(recs),
            "live_driver_uid": live_driver,
            "live_script_name": live_script,
            "canonical_driver_uid": canonical_driver,
            "canonical_script_name": canonical_script,
            "canonical_sd_entry": canonical_sd,
            "canonical_hd_entry": canonical_hd,
            "canonical_assets_present": not missing_assets,
            "invalid_donor_fields": invalid_fields,
        })

    return {
        "ok": True,
        "active_count": len(active),
        "healthy": not any(x.get("fatal_possible") for x in issues),
        "issues": issues,
        "rows": rows,
    }


def repair_managed_state_from_live(game_arg: str | Path,
                                   state_path: str | Path) -> dict[str, Any]:
    """Canonicalize saved scheme identities from the authoritative live DB.

    Only the app state file changes.  Game database bytes are not rewritten.
    Missing canonical asset entries are allowed here because the caller can
    recreate them immediately afterward from the saved source PNG.
    """
    game = base.detect_game(str(game_arg))
    state = load_state(state_path)
    live = base.load_context(str(game))
    ...
    for item in state.get("schemes", []):
        if item.get("superseded_by") or item.get("uid") is None:
            continue
        uid = int(item["uid"])
        recs = live_rows.get(uid, [])
        if len(recs) != 1:
            raise ValueError(f"UID {uid} has {len(recs)} live LIVERIE_c records; state cannot be safely rebound")
        rec = recs[0]
        script = str(base.display(live, rec.fields.get("ScriptName")) or "")
        driver_uid = int(base.field_uid(live, rec, "Driver"))
        if not script:
            raise ValueError(f"UID {uid} has an empty live ScriptName")
        proposed.append((item, uid, script, driver_uid,
                         f"LIVERY_{script}.ARC", f"HDLIVERY_{script}.ARC"))

    scripts = [x[2].casefold() for x in proposed]
    assets = [name.casefold() for x in proposed for name in (x[4], x[5])]
    if len(scripts) != len(set(scripts)):
        raise ValueError("Live managed schemes do not have unique ScriptNames")
    if len(assets) != len(set(assets)):
        raise ValueError("Live managed schemes do not have unique SD/HD asset names")

    for item, uid, script, driver_uid, sd, hd in proposed:
        before = {k: item.get(k) for k in ("driver_uid", "script_name", "sd_entry", "hd_entry")}
        item["driver_uid"] = driver_uid
        item["script_name"] = script
        item["sd_entry"] = sd
        item["hd_entry"] = hd
        item["identity_family"] = "custom_proven" if script.upper().startswith("CUSTOM_") else item.get("identity_family", "native")
        after = {k: item.get(k) for k in before}
        if before != after:
            item["state_rebound_from_live"] = int(time.time())
            changes.append({"uid": uid, "before": before, "after": after})
    save_state(state_path, state)
    return {"ok": True, "changed": len(changes), "changes": changes}


def repair_missing_managed_database_from_live_base(
    game_arg: str | Path,
    state_path: str | Path,
    *,
    donor_uid: int = PROVEN_EXTRA_DONOR_UID,
) -> dict[str, Any]:
    """Add only missing managed records to the current live DB revision.

    Unlike the legacy clean-base repair, this starts from the live database and
    therefore preserves schedules, team links, names, and every unrelated
    app/user edit byte-for-byte. Existing malformed managed records are refused
    rather than duplicated.
    """
    game = base.detect_game(str(game_arg))
    audit = inspect_managed_database(game, state_path)
    missing_uids = {int(x["uid"]) for x in audit["issues"]
                    if x["code"] == "live_livery_missing"}
    if not missing_uids:
        return {"ok": True, "changed": False, "added": 0,
                "note": "All managed live livery records are already present."}
    hard = [x for x in audit["issues"]
            if x.get("fatal_possible", True) and x["code"] not in {
                "live_livery_missing", "canonical_assets_missing",
                "state_live_script_mismatch", "state_live_driver_mismatch",
                "state_asset_name_mismatch",
            }]
    if hard:
        raise ValueError("Managed DB has non-additive corruption: " +
                         "; ".join(x["detail"] for x in hard))

    state = load_state(state_path)
    active = [x for x in state.get("schemes", [])
              if not x.get("superseded_by") and int(x.get("uid", -1)) in missing_uids]
    ctx = base.load_context(str(game))
    before_rows = [(r.class_name, base.pointer_int(ctx, r.uid), _record_sig(ctx, r))
                   for r in ctx.records]
    steps = []
    for item in sorted(active, key=lambda x: int(x.get("created", 0))):
        rebuilt, meta = build_db_with_scheme(
            ctx, int(donor_uid), int(item["driver_uid"]),
            int(item["uid"]), str(item["script_name"]))
        root = ctx.mapper.parse_pyc(rebuilt)
        schemas = ctx.mapper.build_schemas(root)
        records = ctx.mapper.map_records(root, schemas)
        ctx = base.Context(game, ctx.archive, ctx.cdfiles, ctx.row, rebuilt,
                           ctx.mapper, {}, root, schemas, records)
        steps.append(meta)

    expected_prefix = before_rows
    after_without_added = [(r.class_name, base.pointer_int(ctx, r.uid), _record_sig(ctx, r))
                           for r in ctx.records
                           if not (r.class_name == "LIVERIE_c" and
                                   int(base.pointer_int(ctx, r.uid)) in missing_uids)]
    if after_without_added != expected_prefix:
        raise ValueError("Live-base managed DB repair changed an existing record")

    old_size = ctx.archive.stat().st_size
    old_cdf = ctx.cdfiles.read_bytes()
    new_off = v06.align(old_size, ALIGN0)
    rebuilt_cdf = v06.patch_cdf0_bytes(ctx.cdfiles, ctx.row, new_off, len(ctx.pyc))
    try:
        _append(ctx.archive, new_off, ctx.pyc, ALIGN0)
        _atomic(ctx.cdfiles, rebuilt_cdf)
        check = base.load_context(str(game))
        check_existing = [(r.class_name, base.pointer_int(check, r.uid), _record_sig(check, r))
                          for r in check.records
                          if not (r.class_name == "LIVERIE_c" and
                                  int(base.pointer_int(check, r.uid)) in missing_uids)]
        if check_existing != before_rows:
            raise ValueError("Installed live-base DB repair changed an existing record")
    except Exception as original_error:
        rollback_errors = []
        try:
            _truncate(ctx.archive, old_size)
        ...
        if rollback_errors:
            raise RuntimeError(str(original_error) + '; rollback failed: ' + '; '.join(rollback_errors)) from original_error
        raise
    return {"ok": True, "changed": True, "added": len(active),
            "offset": new_off, "size": len(ctx.pyc), "steps": steps}


def _root_constant_spans(pyc: bytes) -> tuple[dict[str, int], list[tuple[int, int]]]:
    """Return the byte span of each root-code constant in a Python-2 PYC."""
    layout = base.root_layout(pyc)
    reader = base.MarshalSkip(pyc)
    reader.pos = int(layout["const_start"])
    spans: list[tuple[int, int]] = []
    for _ in range(int(layout["count"])):
        start = reader.pos
        reader.obj(1)
        spans.append((start, reader.pos))
    if reader.pos != int(layout["const_end"]):
        raise ValueError("Root constant-table span did not match the parsed PYC layout")
    return layout, spans


def _managed_applypatch_blocks(ctx: Any) -> list[dict[str, Any]]:
    """Locate app-created patch-mode livery blocks in both proven layouts.

    Older working builds ended with ``ApplyPatch(DATA)`` after separate
    constructor/post-assignment writes. Newer working builds construct the
    outer patch map inline and call ``ApplyPatch(map)``. Both start by switching
    ``BaseGDTObject_c.__setattr__`` to ``PatchSetAttr`` and end with a one-arg
    ApplyPatch call followed by POP_TOP. Deletion must recognize both shapes.
    """
    code = ctx.root.value.code_bytes
    ins = base.py2_instructions(ctx, code)
    names = ctx.root.value.names

    def opname(i: int) -> str:
        return ins[i]["opname"] if 0 <= i < len(ins) else ""

    def named(i: int, expected: str) -> bool:
        if not (0 <= i < len(ins)):
            return False
        row = ins[i]
        arg = row.get("arg")
        return (row.get("opname") in {"LOAD_NAME", "STORE_ATTR"}
                and arg is not None and 0 <= int(arg) < len(names)
                and names[int(arg)] == expected)

    def is_start(i: int) -> bool:
        return (named(i, "PatchSetAttr") and named(i + 1, "BaseGDTObject_c")
                and named(i + 2, "__setattr__") and opname(i + 2) == "STORE_ATTR")

    starts = [i for i in range(max(0, len(ins) - 2)) if is_start(i)]
    blocks: list[dict[str, Any]] = []
    for pos, i in enumerate(starts):
        boundary = starts[pos + 1] if pos + 1 < len(starts) else len(ins)
        end_i = None
        apply_i = None
        j = i + 3
        while j < boundary:
            if named(j, "ApplyPatch"):
                # The argument may be DATA (legacy exact v0.9 path) or an
                # inline BUILD_MAP result (newer protected path). Find the
                # corresponding one-argument call without assuming its shape.
                k = j + 1
                while k + 1 < boundary:
                    row = ins[k]
                    if (row.get("opname") == "CALL_FUNCTION"
                            and int(row.get("arg") or -1) == 1
                            and opname(k + 1) == "POP_TOP"):
                        apply_i = j
                        end_i = k + 1
                        break
                    k += 1
            ...
            j += 1
        if end_i is None:
            raise ValueError("Found an incomplete app-created ApplyPatch block")
        end_off = ins[end_i + 1]["offset"] if end_i + 1 < len(ins) else len(code)
        blocks.append({
            "start_i": i,
            "end_i": end_i,
            "apply_i": apply_i,
            "start": int(ins[i]["offset"]),
            "end": int(end_off),
            "instructions": ins[i:end_i + 1],
            "layout": ("applypatch_data" if apply_i is not None
                       and named(apply_i + 1, "DATA") else "applypatch_inline_map"),
        })
    return blocks


def _managed_block_for_scheme(ctx: Any, uid: int) -> tuple[Any, str, dict[str, Any], set[int], set[int]]:
    uid = int(uid)
    target_records = [r for r in base.records_of(ctx, "LIVERIE_c")
                      if int(base.pointer_int(ctx, r.uid)) == uid]
    if len(target_records) != 1:
        raise ValueError(f"UID {uid} has {len(target_records)} live LIVERIE records; delete refused")
    target = target_records[0]
    script = str(base.display(ctx, target.fields.get("ScriptName")) or "")
    if not script:
        raise ValueError(f"UID {uid} has no live ScriptName")

    matches = []
    for block in _managed_applypatch_blocks(ctx):
        uid_args: set[int] = set()
        script_args: set[int] = set()
        for row in block["instructions"]:
            if row.get("opname") != "LOAD_CONST" or row.get("arg") is None:
                continue
            arg = int(row["arg"])
            value = base.const_plain(ctx, arg)
            if value == uid:
                uid_args.add(arg)
            if value == script:
                script_args.add(arg)
        if uid_args and script_args:
            matches.append((block, uid_args, script_args))
    if len(matches) != 1:
        layouts = [b.get("layout") for b in _managed_applypatch_blocks(ctx)]
        raise ValueError(
            f"Could not isolate exactly one app-created bytecode block for UID {uid} "
            f"(found {len(matches)} matches across {len(layouts)} patch blocks: {layouts})")
    block, uid_args, script_args = matches[0]
    return target, script, block, uid_args, script_args


def build_db_without_managed_scheme_tombstone(ctx: Any, uid: int) -> tuple[bytes, dict[str, Any]]:
    """Disable one managed livery block in place without touching constants.

    This fallback preserves the complete PYC byte layout and every unrelated
    constant/index. The isolated patch block is replaced byte-for-byte with
    Python-2 NOP opcodes, then the mapper proves that only the requested livery
    record disappeared. It is intentionally used when compact removal cannot
    safely normalize an older working block layout.
    """
    uid = int(uid)
    _target, script, block, uid_args, script_args = _managed_block_for_scheme(ctx, uid)
    block_start = int(block["start"])
    block_end = int(block["end"])
    if block_end <= block_start:
        raise ValueError("The target bytecode block has an invalid length")

    layout = base.root_layout(ctx.pyc)
    out = bytearray(ctx.pyc)
    absolute_start = int(layout["code_off"]) + block_start
    absolute_end = int(layout["code_off"]) + block_end
    out[absolute_start:absolute_end] = bytes([9]) * (absolute_end - absolute_start)  # Python 2 NOP
    rebuilt = bytes(out)
    ...
    schemas2 = mapper.build_schemas(root2)
    records2 = mapper.map_records(root2, schemas2)
    after = base.Context(ctx.game, ctx.archive, ctx.cdfiles, ctx.row, rebuilt,
                         mapper, ctx.containers, root2, schemas2, records2)

    before_rows = [(r.class_name, base.pointer_int(ctx, r.uid), _record_sig(ctx, r))
                   for r in ctx.records
                   if not (r.class_name == "LIVERIE_c" and int(base.pointer_int(ctx, r.uid)) == uid)]
    after_rows = [(r.class_name, base.pointer_int(after, r.uid), _record_sig(after, r))
                  for r in after.records]
    if after_rows != before_rows:
        raise ValueError("NOP live-base delete changed a record other than the requested livery")
    if any(r.class_name == "LIVERIE_c" and int(base.pointer_int(after, r.uid)) == uid
           for r in after.records):
        raise ValueError("NOP live-base delete readback still contains the requested livery")
    return rebuilt, {
        "uid": uid,
        "script_name": script,
        "code_start": block_start,
        "code_end": block_end,
        "code_bytes_nopped": block_end - block_start,
        "uid_const_indexes": sorted(uid_args),
        "script_const_indexes": sorted(script_args),
        "constants_removed": 0,
        "block_layout": block.get("layout"),
        "strategy": "in_place_nop_tombstone",
    }


def build_db_without_managed_scheme(ctx: Any, uid: int) -> tuple[bytes, dict[str, Any]]:
    """Remove exactly one app-created LIVERIE patch from the current live PYC.

    This is the inverse of ``build_db_with_scheme``. It starts from the current
    live database, removes only the matching root ApplyPatch bytecode block and
    its two appended constants, then reparses the full database and proves every
    other mapped record is unchanged. That preserves team moves, manufacturer
    edits, schedules, names, and other unrelated database work.
    """
    uid = int(uid)
    _target, script, block, uid_args, script_args = _managed_block_for_scheme(ctx, uid)
    if len(uid_args) != 1 or len(script_args) != 1:
        raise ValueError("The target patch does not use one UID constant and one ScriptName constant")
    uid_idx = next(iter(uid_args)); script_idx = next(iter(script_args))
    if script_idx != uid_idx + 1:
        raise ValueError("The target patch constants are not the protected UID/ScriptName pair")

    layout, spans = _root_constant_spans(ctx.pyc)
    if not (0 <= uid_idx < len(spans) and 0 <= script_idx < len(spans)):
        raise ValueError("The target patch constant indexes are outside the root constant table")

    # No remaining instruction may reference either removed constant. Later
    # added-paint patches may reference higher constants; those indexes are
    # shifted down by two before the pair is deleted.
    all_ins = base.py2_instructions(ctx, ctx.root.value.code_bytes)
    out = bytearray(ctx.pyc)
    block_start = int(block["start"]); block_end = int(block["end"])
    for row in all_ins:
        if row.get("opname") != "LOAD_CONST" or row.get("arg") is None:
            continue
        off = int(row["offset"]); arg = int(row["arg"])
        if block_start <= off < block_end:
            continue
        if arg in (uid_idx, script_idx):
            raise ValueError("A remaining bytecode instruction references the target scheme constants")
        shift = int(arg > uid_idx) + int(arg > script_idx)
        if shift:
            new_arg = arg - shift
            if not 0 <= new_arg <= 0xFFFF or row.get("arg_pos") is None:
                raise ValueError("A shifted root constant operand is outside the supported range")
            struct.pack_into("<H", out, int(layout["code_off"]) + int(row["arg_pos"]), new_arg)

    block_len = block_end - block_start
    if block_len <= 0:
        raise ValueError("The target bytecode block has an invalid length")
    struct.pack_into("<i", out, int(layout["code_len_pos"]), int(layout["code_len"]) - block_len)
    struct.pack_into("<i", out, int(layout["count_pos"]), int(layout["count"]) - 2)

    # Constants are after root code, so delete them first while their original
    # absolute spans are still valid, then delete the root-code block.
    for idx in sorted((uid_idx, script_idx), reverse=True):
        start, end = spans[idx]
        del out[start:end]
    absolute_start = int(layout["code_off"]) + block_start
    absolute_end = int(layout["code_off"]) + block_end
    del out[absolute_start:absolute_end]
    rebuilt = bytes(out)
    ...
    after_rows = [(r.class_name, base.pointer_int(after, r.uid), _record_sig(after, r))
                  for r in after.records]
    if after_rows != before_rows:
        raise ValueError("Live-base delete changed a record other than the requested livery")
    if any(r.class_name == "LIVERIE_c" and int(base.pointer_int(after, r.uid)) == uid
           for r in after.records):
        raise ValueError("Live-base delete readback still contains the requested livery")
    return rebuilt, {
        "uid": uid,
        "script_name": script,
        "code_start": block_start,
        "code_end": block_end,
        "code_bytes_removed": block_len,
        "uid_const_index": uid_idx,
        "script_const_index": script_idx,
        "constants_removed": 2,
    }


def remove_managed_livery_from_live_base(game_arg: str | Path, uid: int) -> dict[str, Any]:
    """Install a live-database revision with one app-created livery removed."""
    game = base.detect_game(str(game_arg))
    ctx = base.load_context(str(game))
    compact_error = None
    try:
        rebuilt, meta = build_db_without_managed_scheme(ctx, int(uid))
        meta["strategy"] = "compact_block_and_constant_delete"
    except Exception as ex:
        compact_error = str(ex)
        try:
            rebuilt, meta = build_db_without_managed_scheme_tombstone(ctx, int(uid))
            meta["compact_delete_unavailable"] = compact_error
        except Exception as tombstone_error:
            raise ValueError(
                "Could not surgically remove the live added-scheme record. "
                f"Compact delete: {compact_error} | In-place NOP delete: {tombstone_error}"
    ...
    try:
        ...
        _atomic(ctx.cdfiles, rebuilt_cdf)
        check = base.load_context(str(game))
        if check.pyc != rebuilt:
            raise ValueError("Live-base delete database readback mismatch")
        if any(r.class_name == "LIVERIE_c" and int(base.pointer_int(check, r.uid)) == int(uid)
               for r in check.records):
            raise ValueError("Deleted livery UID remains present after readback")
    except Exception as original_error:
        rollback_errors = []
        try:
            _truncate(ctx.archive, old_size)
    ...
    return {
        ...
        "sha256": _sha(rebuilt),
        "meta": meta,
        "preserved_unrelated_records": True,
    }
...
def rebuild_managed_database_from_clean_base(
    ...
    *,
    ...
    game = base.detect_game(str(game_arg))
    state = load_state(state_path)
    all_state_schemes = [x for x in state.get("schemes", []) if x.get("uid") is not None]
    active = [x for x in all_state_schemes if not x.get("superseded_by")]
    # Zero active schemes is valid for the safe remove-slot path. Superseded
    # UIDs are still excluded from the live-vs-clean comparison because they
    # are precisely the app-created records this rebuild is removing.
    managed_uids = {int(x["uid"]) for x in active}
    all_managed_uids = {int(x["uid"]) for x in all_state_schemes}
    live = base.load_context(str(game))
    clean = _context_from_indexed_pyc(game, Path(backup_archive), Path(backup_cdf))
    if _rows_without_managed(live, all_managed_uids) != _rows_without_managed(clean, set()):
        raise ValueError(
