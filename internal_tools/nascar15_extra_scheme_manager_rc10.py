...
import nascar15_inrange_true_extra_scheme_probe_v0_4_base_rc10 as base
import nascar15_complete_asset_backed_extra_scheme_probe_v0_6_rc10 as v06
import nascar15_patchmode_applypatch_extra_scheme_rc10 as v09
import nascar15_true_extra_scheme_preview_persistence_rc10 as v10
import nascar15_ai_scheme_named_event_multiseries_rc10 as ev5
...
STOCK_MAX_SCRIPT_NAME = 31
STOCK_MAX_SD_CDF_NAME = 42
STOCK_MAX_HD_CDF_NAME = 44


def _compact_runtime_identity(stem: str, driver_uid: int, new_uid: int) -> str:
    """Build a stock-bounded livery identity for the native runtime.

    The clean mapped game contains no LIVERIE_c ScriptName longer than 31
    characters.  The historically successful manual v0.9 identity was the
    compact ``15_47_AJ_EXTRA_SLOT_TEST``; the generalized app accidentally
    expanded that into identities as long as 40 characters before the game
    reached SetLiveryName().  Keep the runtime identity inside the measured
    stock envelope while retaining the year, car number, a driver token, and
    the unique sub-25600 livery UID.
    """
    tokens = [x for x in re.sub(r"[^A-Z0-9_]+", "_", str(stem).upper()).split("_") if x]
    if len(tokens) >= 3 and tokens[0].isdigit() and tokens[1].isdigit():
        prefix = "_".join(tokens[:3])
    else:
        prefix = f"15_DRIVER_{int(driver_uid)}"
    suffix = f"_EXTRA_{int(new_uid)}"
    room = STOCK_MAX_SCRIPT_NAME - len(suffix)
    if room < 8:
        raise ValueError("The stock ScriptName envelope cannot hold this livery UID")
    prefix = prefix[:room].rstrip("_")
    script = prefix + suffix
    if len(script) > STOCK_MAX_SCRIPT_NAME:
        raise ValueError("Generated ScriptName exceeds the measured stock limit")
    if len(f"LIVERY_{script}.ARC") > STOCK_MAX_SD_CDF_NAME:
        raise ValueError("Generated SD livery entry exceeds the measured stock limit")
    if len(f"HDLIVERY_{script}.ARC") > STOCK_MAX_HD_CDF_NAME:
        raise ValueError("Generated HD livery entry exceeds the measured stock limit")
    return script
...
def suggest_identity(
    ...
    """Return a sub-25600 identity inside the measured stock name envelope."""
    ...
    script = _compact_runtime_identity(stem, int(driver_uid), int(uid))
    return {
        "uid": int(uid),
        "script_name": script,
        "identity_family": "stock_bounded_v09_compact",
        "script_name_length": len(script),
        "sd_entry_length": len(f"LIVERY_{script}.ARC"),
        "hd_entry_length": len(f"HDLIVERY_{script}.ARC"),
        "stock_max_script_name": STOCK_MAX_SCRIPT_NAME,
    }
...
def build_db_with_scheme(ctx: Any, donor_uid: int, driver_uid: int, new_uid: int, script_name: str) -> tuple[bytes, dict[str, Any]]:
    ...
    patch_code, patch_meta, encoded_constants = v09.build_applypatch_code(
        ctx, donor, int(new_uid), script_name, int(driver_uid)
    )
