...
The allocator is team-agnostic; callers must enforce the release scope and in-game safety gate.
"""
...
VERSION = "1.5-canonical-thumbnail-test"
...
def _native_encoder(image, fmt="DXT5"):
    # The packaged texconv is preferred on Windows; containers.py supplies the
    # deterministic DXT5 fallback for test/repair environments.
    return thumb.texconv_encode(image, fmt)


def _apply_native_thumbnail_pixels(raw: bytes, resource_name: str,
                                   image_path: str | Path) -> tuple[bytes, str, dict[str, Any]]:
    entries, _ = C.parse_multi_arc(raw)
    target = next((e for e in entries if e["name"] == resource_name), None)
    if target is None:
        raise ValueError(f"canonical native resource not found: {resource_name}")
    if (int(target["w"]), int(target["h"]), str(target["fmt"])) != (256, 256, "DXT5"):
        raise ValueError(
            f"{resource_name} is {target['w']}x{target['h']} {target['fmt']}; "
            "expected native 256x256 DXT5")
    ...
    rebuilt = C.multi_write_png_validated(
        raw, target, image, encode_fn=_native_encoder)
    check_entries, _ = C.parse_multi_arc(rebuilt)
    check = next(e for e in check_entries if e["name"] == resource_name)
    C.multi_read_png(rebuilt, check)
    return rebuilt, "canonical native DXT5 PNG import (test)", {
        "payload_abs": int(target["payload_abs"]),
        "payload_size": int(target["payload_size"]),
        "layout": str(target.get("layout")),
        "dxt5_swapped": bool(target.get("dxt5_swapped")),
        "pixel_readback_verified": True,
    }
...
def _backup_pair(data: Path) -> tuple[Path, Path] | None:
    # Release lineage is authoritative; timestamps are not. ZIP extraction,
    # cloud sync and copied installs can make an older pristine backup look
    # newer than an already-modded backup.
...
def _choose_template(
    ...
    *,
    ...
    new_name: str,
    require_alternate_paint_donor: bool = False,
):
    ...
    for raw, template, source_label, container_name, is_current in _template_candidates(
        ...
        try:
            ...
            continue
        donor_match = re.fullmatch(r"PAINTSCHEME_([0-9]+)", str(donor.name), re.I)
        donor_uid = int(donor_match.group(1)) if donor_match else -1
        if require_alternate_paint_donor and not (25500 <= donor_uid < 25600):
            rejected.append(
                f"{container_name}: new slot donor {donor.name} is not a native 255xx alternate identity"
            )
            continue
        remaining_paint = sum(
            1 for e in unused
...
def plan_fixed_template_thumbnail(
    game_arg: str | Path,
    *,
    target_container: str,
    team_driver_uids: Iterable[int],
    ...
    """Return the exact fixed-template plan without modifying any file."""
    ...
    (score, template_raw, template, template_source, selected_template,
     mapping, donor, unused, remaining_paint, driver_pair_map) = _choose_template(
        game,
        recipient_raw=recipient_raw,
        recipient=recipient,
        target_container=target_container,
        sources=sources,
        new_name=new_name,
        require_alternate_paint_donor=bool(require_alternate_paint_donor),
    )
    return {
        "ok": True,
        "version": VERSION,
        "read_only": True,
        "target_container": target_container,
        "recipient_count": int(recipient.count),
        "required_existing_resources": [str(e.name) for e in sources],
        "required_resource_count_before_new": len(sources),
        "new_resource_name": new_name,
        "selected_template": selected_template,
        "template_source": template_source,
        "is_current_target": bool(selected_template == target_container and template_source == "current target bank"),
        "template_count": int(template.count),
        "template_size": len(template_raw),
        "score": list(score),
        "new_thumbnail_identity_donor": str(donor.name),
        "required_alternate_paint_donor": bool(require_alternate_paint_donor),
        "remaining_native_paint_slots_after_plan": int(remaining_paint),
        "driver_pair_map": driver_pair_map,
        "paired_driver_art": True,
        "resource_mapping": [
            {"destination_identity": str(dest.name), "source_resource": str(source.name)}
            for dest, source in mapping
        ],
        "unused_resource_names": [str(e.name) for e in unused],
    }
...
def install_fixed_template_thumbnail(
    ...
    *,
    ...
    # Clean-capture test path: the public build's PNG writer was not merely
    # "unsafe DXT5"; it began every payload 40 bytes late and swapped standard
    # DXT5 halves.  Write only the canonical native +24 payload, then prove that
    # every wrapper, directory record, name, and neighboring texture is unchanged.
    final, encoder, pixel_report = _apply_native_thumbnail_pixels(
        rebuilt, new_name, image_path)
    ...
    return {
        ...
        "game_safe_stock_legacy": False,
        # FIX (v1.0.2-dev11): this path DOES write the caller's thumbnail --
        # _apply_native_thumbnail_pixels imports it at the canonical +24 payload
        # offset, re-parses, and decodes it back before returning.  The flag was
        # nevertheless hardcoded False, so _extra_update_preview_state computed
        # thumbnail_game_safe=False and the UI reported "Thumbnail identity needs
        # repair" for every newly created slot, forever.  Report what actually
        # happened: safe when the pixel import verified its own readback.
        "game_safe_same_bank_custom": bool(pixel_report.get("pixel_readback_verified")),
        "custom_image_installed": True,
        "game_safe_raw_clone": False,
        "canonical_pixel_test": True,
