...
def parse_multi(arc: bytes):
    """Return the canonical native texture entries for one ARCC bank.

    NASCAR 15 banks use 16-byte physical records and a 24-byte texture header.
    The old research probe treated pairs as synthetic 32-byte logical records,
    which shifted every payload by 40 bytes and truncated the final blocks.
    Keep structural expansion in the v10 builder; pixel reads/writes use the
    canonical parser here.
    """
...
def encode_target(image: Image.Image, entry):
    ...
    # Native NASCAR 15 primary16 banks store ordinary DXT5 block order.  Only
    # the legacy fallback layout needs the historical 8-byte half swap.
    if entry.get("dxt5_swapped"):
        enc=swap_dxt5(enc)
    return enc,encoder
...
def replace_payload(arc: bytes, entry, image: Image.Image):
    ...
    pa,ps=int(entry["payload_abs"]),int(entry["payload_size"])
    ...
    if t.get("dxt5_swapped"):
        pay=swap_dxt5(pay)
    dxt5_decode(pay,t["w"],t["h"])
    return out,encoder
